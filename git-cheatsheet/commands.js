...
var translations = {
  en: {
    ...
    commands: {
      "status": {"cmd": "status", "docs": "Visualiza las rutas que tienen diferencias entre ficheros del índice y la acometida actual de la HEAD, rutas que tienen diferencias entre el espacio de trabajo y el fichero del índice, y rutas en el espacio de trabajo que no son seguidas por git."},
      "diff": {"cmd": "diff", "docs": "Visualiza las diferencias no añadidas al índice."},
      "diff x": {"cmd": "diff <acometida o rama>", "docs": "Visualiza los cambios quetienes en tu espacio de trabajo relativos a la mencionada <acometida>. Puedes usar la HEAD para compararlo con la última acometida, o un nombre de rama la cual comparar con el extremo de otra rama diferente"},
      "add x": {"cmd": "add <fichero... o directorio...>", "docs": "Añade el contenido actual de ficheros nuevos o modificados al índice, preparande de ese modo el contenido para su inclusión en la próxima acometida. Use `add --interactive` para añadir los contenidos modificados del espacio de trabajo interactivamente al índice."},
      "add -u": {"cmd": "add -u", "docs": "Añade el contenido actual o modificado (NO NUEVO) de los ficheros al índice.  Esto es similar a lo que hace 'git commit -a' al prepararse para hacer una acometida."},
      "rm x": {"cmd": "rm <fichero(s)...>", "docs": "Elimina un fichero del espacio de trabajo y del índice."},
      "mv x": {"cmd": "mv <fichero(s)...>", "docs": "Mueve un fichero en el espacio de trabajo y en el índice."},
      "commit -a": {"cmd": "commit -a [-m 'mensaje']", "docs": "Acomete todos los ficheros cambiados desde tu última acometida, excepto ficheros no seguidos (p.ej. todos los ficheros ya listados en el índice). Elimina ficheros del índice que hayan sido eliminados del espacio de trabajo."},
      "checkout x": {"cmd": "checkout <fichero(s)... o directorio>", "docs": "Actualiza el fichero o directorio del espacio de trabajo. No cambia ramas."},
      "reset head x": {"cmd": "reset HEAD <file(s)...>", "docs": "Elimina los ficheros especificados de la próxima acometida. Reestablece el índice no solo el árbol del espacio de trabajo (p.ej. los ficheros cambiados se preservan pero no se marcan para acometer) e informa de qué es lo que no ha sido actualizado."},
      "reset --soft head^": {"cmd": "reset --soft HEAD^", "docs": "Undo the last commit, leaving changes in the the index."},
      "reset --hard": {"cmd": "reset --hard", "docs": "Matches the workspace and index to the local tree. WARNING: Any changes to tracked files in the working tree since commit are lost. Use this if merging has resulted in conflicts and you'd like to start over. Pass ORIG_HEAD to undo the most recent successful merge and any changes after."},
      "checkout b": {"cmd": "checkout <branch>", "docs": "Switches branches by updating the index and workspace to reflect the specified branch, <branch>, and updating HEAD to be <branch>."},
      "checkout -b x": {"cmd": "checkout -b <name of new branch>", "docs": "Create a branch and switch to it"},
